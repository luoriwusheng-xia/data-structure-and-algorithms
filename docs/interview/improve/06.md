...
![img](./06-image/20210328112610.webp)
...
- 新增`Composition API`，更好的逻辑复用和代码组织;
  - vue2 主要使用选项式 API（Options API），将不同的逻辑关注点分散在 data、methods、computed、watch 等选项中。当组件变得复杂时，代码会变得难以维护，逻辑碎片化严重
...
```
- vue3 引入了组合式 API（Composition API），允许开发者根据逻辑功能来组织代码，将相关的逻辑封装在一起，提高了代码的复用性和可维护性。
...
虚拟 DOM 渲染和更新的性能有一定的提升空间，在处理大型项目和复杂组件时可能会出现性能瓶颈。

Vue 3

重写了虚拟 DOM 实现，采用了静态提升、Patch Flag 等技术，显著提高了渲染和更新的性能。静态提升可以将模板中的静态节点提取出来，避免重复渲染；Patch Flag 可以标记动态节点，让虚拟 DOM 只更新有变化的部分。

- <BoldText inline>静态提升</BoldText>：Vue3 会分析模板，将其中的静态节点提升到渲染函数之外，只在初始化时创建一次，而不是每次渲染都重新创建。比如在一个包含大量静态文本和图片的列表组件中，Vue2 可能会在每次更新时都重新创建这些静态节点的虚拟 DOM，而 Vue3 只会创建一次，大大减少了渲染的工作量。
- <BoldText inline>Patch Flag</BoldText>：Vue3 为每个节点添加了 Patch Flag，标记节点的更新类型，比如是文本更新、属性更新还是事件绑定更新等。在更新时，只需要对比和更新带有相应标志的节点，避免了对整个虚拟 DOM 树的遍历和对比。以一个包含多个输入框和按钮的表单组件为例，当用户只修改了一个输入框的值时，Vue3 可以通过 Patch Flag 精准地找到该输入框对应的节点进行更新，而不会浪费时间在其他未变化的节点上。
- <BoldText inline>动态节点重用</BoldText>：在 Vue3 中，如果动态节点的顺序发生了变化，但节点本身的类型和属性没有改变，Vue3 会尝试重用已有的节点，而不是重新创建。比如在一个可排序的列表中，用户拖动列表项改变顺序，Vue3 可以高效地重用这些列表项节点，只需要更新它们的位置，提高了更新效率。

**4. 类型支持**

使用Typescript替换Flow， vue2.6 使用 flow, vue3使用 Typescript

- vue2 对 TypeScript 的支持相对较弱，使用 TypeScript 时需要编写大量的类型声明，开发体验不够流畅。
- vue3 从设计上就对 TypeScript 有了更好的支持，组合式 API 天然与 TypeScript 配合良好，开发者可以更方便地使用类型系统来增强代码的健壮性。


**5. 生命周期钩子**

- vue2 有 beforeCreate、created、beforeMount、mounted、beforeUpdate、updated、beforeDestroy、destroyed 等生命周期钩子。
- vue3 保留了大部分生命周期钩子，但名称有所变化，如 beforeDestroy 改为 beforeUnmount，destroyed 改为 unmounted。同时，配合组合式 API 使用时，生命周期钩子以函数形式导入。
...
**6. 多根节点组件**
- vue2 组件模板必须有一个根节点，否则会报错。
- vue3 支持多根节点组件，组件模板可以有多个根节点。

**7. 语法糖上的改变**

- vue2中 v-for优先级高于 v-if; vue3中 v-if优先级高于 v-for；
- vue2中 v-for和v-if **可以**作用在同一个元素上， vue3中v-for和v-if**不能**作用在同一个元素上,会报错

<<<./code/06/19-1.vue

- setup 语法糖 - 本质是宏编译
- setup 中的组件和指令无需导出，可直接使用
- setup 默认是封闭的， 需要通过 deinfeExpose 来暴露给外部使用
- vue3支持多v-model 绑定
