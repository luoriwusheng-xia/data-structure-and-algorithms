...
## 4 实现new的过程
- [new-操作符具体干了什么呢](/docs/interview/base/03/#_8-new-操作符具体干了什么呢)
...
1. 什么是 new.target？
new.target 是一个元属性（Meta Property），它允许你检测函数或构造函数是否是通过 new 关键字调用的。在构造函数内部，new.target 指向被调用的构造函数。如果函数不是通过 new 关键字调用，new.target 的值为 undefined。
2. 如何使用 new.target？
以下是一个使用 new.target 的简单示例：

<<<./code/01/07-5.js

在这个示例中，Person 函数检查 new.target 的值。如果是通过 new 关键字调用，它会正常创建对象并设置 name 属性；如果不是，则会抛出一个错误。

3. new.target 的来源

new.target 是为了解决在构造函数中无法直接判断是否使用 new 关键字调用的问题而引入的。

在 new.target 出现之前，为了确保一个函数作为构造函数使用，通常会采用一些变通的方法，比如使用 instanceof 或者在函数内部检查 this 是否为该函数的实例，但这些方法都有一定的局限性。new.target 提供了一种简洁和准确的方式来判断函数是否被作为构造函数使用。

4. 引入的 JavaScript 版本
new.target 是在 ECMAScript 2015（ES6）中引入的。它是 ES6 众多新特性之一，旨在让 JavaScript 语言更具表达力和安全性。

5. 应用场景
强制使用 new 关键字：如上述示例，可强制用户使用 new 关键字调用构造函数，以确保函数正确的行为。
创建可继承的工厂函数：在工厂函数中，可以根据 new.target 的值来决定返回的对象是使用 new 创建的实例还是普通对象，这样可以方便地创建可以被继承的工厂函数。

<<<./code/01/07-6.js

在这个示例中，当使用 new 调用 createObject 时，会创建一个实例对象，而不使用 new 时，会返回一个普通对象。

6. 注意事项

- 只能在函数或类的内部使用 new.target，在函数或类外部使用会导致语法错误。
- 对于箭头函数，new.target 总是 undefined，因为箭头函数不能作为构造函数使用。

总之，new.target 为开发人员提供了一种简单、直接的方式来检查函数是否使用 new 关键字调用，有助于提高代码的健壮性和可维护性，特别是在创建和管理对象实例时，它可以帮助你编写更安全、更灵活的代码。

## 8 实现深拷贝

- [lodash-源码-深拷贝](/docs/source-code/lodash/language/#简易版深拷贝)
...
promisifyNew
  .readFileAsync(path.join(__dirname, './promise.js'), 'utf8')
  .then((d) => {
    console.log(d);
  });
...
## 箭头函数，普通函数，Generator 函数区分

下是解决该问题的思路：

1. 首先，需要明确三种函数的特点：
- 普通函数：使用 function 关键字定义，可以使用 new 关键字进行实例化（构造函数），this 的指向取决于函数的调用方式。
- 箭头函数：使用 => 语法定义，没有自己的 this，this 是继承自外部作用域，不能用作构造函数，没有 arguments 对象。
- Generator 函数：使用 function* 关键字定义，内部可以使用 yield 关键字暂停函数的执行并返回一个中间结果，调用 Generator 函数会返回一个 Generator 对象，可以通过 next() 方法逐步执行函数。

2. 然后，根据这些特点编写判断函数：
- 对于普通函数，可以检查是否使用 function 关键字定义，并且不是 Generator 函数。
- 对于箭头函数，可以检查是否使用 => 符号定义。
- 对于 Generator 函数，可以检查是否使用 function* 关键字定义。
