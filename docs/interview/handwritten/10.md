...
- [领导：写个请求库，要支持中断请求、缓存请求（幂等）、重试请求、并发控制、脚手架生成代码模板... - 掘金](https://juejin.cn/post/7359821944148361256)
...
<<<./code/10/04-1.js [异步串行]

<<<./code/10/04-2.js [异步并行]
:::
...
### 14 查找数组公共前缀（美团）
- [leetcode - 14题](https://leetcode.cn/problems/longest-common-prefix/)
...
- 对于一个字符串数组，该算法首先假设第一个字符串是公共前缀。
- 然后依次扫描数组中的每个字符串，不断更新公共前缀，使其成为当前公共前缀和下一个字符串的公共前缀。
- 如果当前公共前缀与下一个字符串没有公共前缀，公共前缀的长度会逐渐减小，直到找到公共前缀或者公共前缀长度为 0。

**优缺点**
- 优点：
  - 代码简单直观，易于理解和实现，不需要额外的数据结构。
  - 对于较短的字符串数组，性能通常是可以接受的。
- 缺点：
  - 时间复杂度较高，在最坏情况下为 ，其中 m 是字符串的平均长度，n 是数组中字符串的数量。

**分治算法：**
> 对应 14-2.js

分治算法的核心思想是将一个大问题分解为几个较小的子问题，解决这些子问题，然后将子问题的解组合起来得到原问题的解。
在这个查找公共前缀的递归函数 longestCommonPrefixRecursive 中：
- 分：将查找整个数组的公共前缀问题分解为查找前两个元素的公共前缀问题，这通过 helper 函数完成。
- 治：通过 helper 函数找到前两个元素的公共前缀，得到 prefix。
- 组合：将得到的 prefix 替换数组的前两个元素，然后递归调用 longestCommonPrefixRecursive 函数，将新的数组（长度减少了一个元素）作为新的输入，继续求解公共前缀，直到最终得到结果。


### 15 判断括号字符串是否有效（小米）
- [leetcode 20题](https://leetcode.cn/problems/valid-parentheses/description/)
...
时间复杂度：
- 该算法的时间复杂度是 ，其中 n 是输入字符串 s 的长度，因为我们只对字符串进行一次遍历。

空间复杂度：
- 空间复杂度是 ，因为在最坏情况下，输入字符串中的所有字符都是左括号，需要将它们全部存储在栈中。

这个算法利用栈的后进先出（LIFO）特性，巧妙地解决了括号匹配问题，通过存储左括号并在遇到右括号时进行匹配检查，确保括号序列的有效性。
