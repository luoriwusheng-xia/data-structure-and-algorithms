...
*   接下来我们通过 `babylon` 解析代码获取
`AST` ，目的是为了分析代码中是否还引入了别的文件

*   通过 `dependencies` 来存储文件中的依赖，然后再将 `AST` 转换为 `ES5`
    代码
...
*   如果是 `CSS` 文件的话，我们就不能用 `Babel` 去编译了，只需要读取
`CSS` 文件中的代码，然后创建一个 `style`
...
> `Babel` 将我们 `ES6` 的模块化代码转换为了
> `CommonJS` 的代码，但是浏览器是不支持 `CommonJS`

> 的，所以如果这段代码需要在浏览器环境下运行的话，我们需要自己实现
> `CommonJS` 相关的代码，这就是 `bundle` 函数做的大部分事情。
...
`require` 函数，然后调用 `require(entry)` ，也就是
`require('./entry.js')` ，这样就会从函数参数中找到 `./entry.js`
