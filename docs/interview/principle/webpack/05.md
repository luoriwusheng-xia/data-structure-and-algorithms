...
options文档](https://babeljs.io/docs/usage/api/#options)
...
*   env
    ...
    另外，还有
    stage-0到stage-4的标准成形之前的各个阶段，这些都是实验版的preset，建议不要使用。
...
2.  要确保**在入口处导入polyfill**，因为polyfill代码需要在所有其他代码前先被调用
    代码方式： `import "babel-polyfill"` webpack配置：
...
> -   最后补充一点：由于runtime不会污染全局空间，所以**实例方法是无法工作的**（因为这必须在原型链上添加这个方法，这是和polyfill最大的不同）
> ，比如：
