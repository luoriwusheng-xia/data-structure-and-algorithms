export default {
  // '/docs/frontend/': [
  //   {
  //     text: '前端',
  //     items: [
  //       {
  //         text: '杂文',
  //         link: '/docs/frontend/index'
  //       }
  //     ]
  //   }
  // ],
  "/docs/frontend/translation/index": [
    {
      text: 'codemirror-中文文档',
      link: '/docs/frontend/translation/codemirror/index'
    },
    {
      text: 'cac-中文文档',
      link: '/docs/frontend/translation/cac/index'
    }
  ],
  '/docs/leetcode/': [
    {
      text: 'leetcode',
      items: [
        { text: '链表', link: '/docs/leetcode/01/index' },
        { text: '数组', link: '/docs/leetcode/02/index' },
        { text: '树', link: '/docs/leetcode/03/index' },
        { text: '排序', link: '/docs/leetcode/04/index' },
        { text: '字符串', link: '/docs/leetcode/05/index' },
        { text: '动态规划', link: '/docs/leetcode/06/index' },
        { text: '位运算', link: '/docs/leetcode/07/index' },
        { text: '双指针', link: '/docs/leetcode/08/index' },
        { text: '素数个数统计', link: '/docs/leetcode/09/index' },
        { text: '数学', link: '/docs/leetcode/10/index' },

        {
          text: '简单',
          items: [
            {
              text: '回文数',
              link: '/docs/leetcode/11/index'
            }
          ]
        },
        {
          text: '中等',
          items: [
            {
              text: '寻找两个正序数组的中位数',
              link: '/docs/leetcode/12/index'
            }
          ]
        }
      ],
    },
  ],

  '/docs/vitepress-config/': [
    {
      text: 'vitepress',
      items: [
        {
          text: '评论功能',
          link: '/docs/vitepress-config/01/index',
        },
        {
          text: '多语言',
          link: '/docs/vitepress-config/02/index',
        },
        {
          text: '主题开发',
          link: '/docs/vitepress-config/03/index',
        },
        {
          text: 'element-plus接入',
          link: '/docs/vitepress-config/04/index',
        },
        {
          text: 'sandpack接入',
          link: '/docs/vitepress-config/05/index',
        },
        {
          text: '使用unocss',
          link: '/docs/vitepress-config/06/index',
        },
        {
          text: '使用vue',
          link: '/docs/vitepress-config/07/index',
        },
        {
          text: '编程式使用 Router 和 Route',
          link: '/docs/vitepress-config/08/index',
        },
        {
          text: '插件开发',
          link: '/docs/vitepress-config/01/index',
        },
        {
          text: '全局搜索',
          link: '/docs/vitepress-config/01/index',
        },
        {
          text: '部署',
          link: '/docs/vitepress-config/01/index',
        },
      ],
    },
  ],

  '/docs/backend/file/': [
    {
      text: '文件分片上传',
      link: '/docs/backend/file/01'
    },
    {
      text: '文件分片下载',
      link: '/docs/backend/file/02'
    }
  ],

  '/docs/backend/jwt/': [
    {
      text: 'jwt登录',
      link: '/docs/backend/jwt/'
    },
    {
      text: 'RBAC',
      link: '/docs/backend/jwt/rbac'
    }
  ],

  '/docs/backend/optimize/01': [
    {
      text: '压测',
      link: '/docs/backend/optimize/01'
    }
  ],
  '/docs/backend/websocket/01': [
    {
      text: 'websocket',
      link: '/docs/backend/websocket/01'
    }
  ],

  '/docs/frontend/essay/': [
    {
      text: '渡一前端',
      items: [
        {
          text: '判断函数是否标记了async',
          link: '/docs/frontend/essay/01/'
        },
        {
          text: '消除异步的传染性',
          link: '/docs/frontend/essay/02/'
        },
        {
          text: '自动检测更新',
          link: '/docs/frontend/essay/03/'
        },
        {
          text: '妙用位运算-算法-找唯一数字',
          link: '/docs/frontend/essay/04/'
        },
        {
          text: '跳出指定for循环',
          link: '/docs/frontend/essay/05/'
        },
        {
          text: '网格布局中的动画',
          link: '/docs/frontend/essay/06/'
        },
        {
          text: '瀑布流布局',
          link: '/docs/frontend/essay/07/'
        },
        {
          text: '统计字符串中字符频率',
          link: '/docs/frontend/essay/08/'
        },
        {
          text: 'HTMLCollection与NodeList区别',
          link: '/docs/frontend/essay/09/'
        },
      ]
    },
    {
      text: '动画特效',
      items: [
        {
          text: 'svg动画',
          link: '/docs/frontend/essay/animation/01/',
        }
      ]
    },
    {
      text: '工具',
      items: [
        {
          text: '将html转成md',
          link: '/docs/frontend/tools/01/'
        },
        {
          text: 'markdown-it',
          link: '/docs/frontend/tools/markdown-it/'
        }
      ]
    },
    {
      text: 'css',
      items: [
        {
          text: '轮播',
          link: '/docs/frontend/essay/css/01/'
        },
        {
          text: '你不需要javascript-系列',
          link: '/docs/frontend/essay/css/02/'
        },
        {
          text: 'grid',
          link: '/docs/frontend/essay/css/03/'
        },
        {
          text: '四角线框的跟随移动',
          link: '/docs/frontend/essay/css/04/'
        },
        {
          text: 'css原生变量',
          link: '/docs/frontend/essay/css/05/'
        }
      ]
    },
    {
      text: '计算机基础',
      items: [
        {
          text: '左移, 右移',
          link: '/docs/frontend/essay/100/'
        }
      ]
    }
  ],


  '/docs/frontend/engineering/index': [
    {
      text: 'webpack与rollup原理与运行时分析',
      link: '/docs/frontend/engineering/01/index'
    },
    {
      text: 'gulp.js',
      link: '/docs/frontend/engineering/02/index'
    }
  ],

  '/docs/source-code/vue2/': [
    {
      text: '源码首页',
      link: '/docs/source-code/vue2/'
    },
    {
      text: '01-',
      link: '/docs/source-code/vue2/01'
    },
    {
      text: '02-',
      link: '/docs/source-code/vue2/02'
    },
    {
      text: '03-响应式原理',
      link: '/docs/source-code/vue2/03'
    },
    {
      text: '04-异步更新',
      link: '/docs/source-code/vue2/04'
    },
    {
      text: '05-全局API实现原理',
      link: '/docs/source-code/vue2/05'
    },
    {
      text: '2.7.14 调试',
      link: '/docs/source-code/vue2/30'
    },
    {
      text: '变更日志',
      link: '/docs/source-code/vue2/changelog'
    }
  ],

  '/docs/interview/base/': [
    {
      "text": "一、html、http、web综合问题",
      "link": "/docs/interview/base/01"
    },
    {
      "text": "二、css相关",
      "link": "/docs/interview/base/02"
    },
    {
      "text": "三、javascript相关",
      "link": "/docs/interview/base/03"
    },
    {
      "text": "四、jquery相关",
      "link": "/docs/interview/base/04"
    },
    {
      "text": "五、bootstrap相关",
      "link": "/docs/interview/base/05"
    },
    {
      "text": "六、微信小程序相关",
      "link": "/docs/interview/base/06"
    },
    {
      "text": "七、webpack相关",
      "link": "/docs/interview/base/07"
    },
    {
      "text": "八、框架相关",
      "link": "/docs/interview/base/08"
    },
    {
      "text": "九、编程题相关",
      "link": "/docs/interview/base/09"
    },
    {
      "text": "十、前端综合问题",
      "link": "/docs/interview/base/10"
    },
    {
      "text": "十一、hr面相关",
      "link": "/docs/interview/base/11"
    }
  ],

  '/docs/interview/improve/': [
    {
      "text": "一、JS基础",
      "link": "/docs/interview/improve/01"
    },
    {
      "text": "二、HTML",
      "link": "/docs/interview/improve/02"
    },
    {
      "text": "三、CSS基础",
      "link": "/docs/interview/improve/03"
    },
    {
      "text": "四、浏览器",
      "link": "/docs/interview/improve/04"
    },
    {
      "text": "五、框架通识",
      "link": "/docs/interview/improve/05"
    },
    {
      "text": "六、Vue",
      "link": "/docs/interview/improve/06"
    },
    {
      "text": "七、React",
      "link": "/docs/interview/improve/07"
    },
    {
      "text": "八、性能",
      "link": "/docs/interview/improve/08"
    },
    {
      "text": "九、工程化",
      "link": "/docs/interview/improve/09"
    },
    {
      "text": "十、HTTP",
      "link": "/docs/interview/improve/10"
    },
    {
      "text": "十一、9种前端常见的设计模式",
      "link": "/docs/interview/improve/11"
    },
    {
      "text": "十二、综合问题",
      "link": "/docs/interview/improve/12"
    },
    {
      "text": "十三、人事面",
      "link": "/docs/interview/improve/13"
    }
  ],
  '/docs/interview/handwritten/': [
    {
      text: '1-27 基础',
      link: '/docs/interview/handwritten/01'
    },
    {
      text: '28 框架相关',
      link: '/docs/interview/handwritten/02'
    },
    {
      text: '29 数组相关',
      link: '/docs/interview/handwritten/03'
    },
    {
      text: '30 正则相关',
      link: '/docs/interview/handwritten/04'
    },
    {
      text: '31 函数柯里化相关',
      link: '/docs/interview/handwritten/05'
    },
    {
      text: '32 字符串相关',
      link: '/docs/interview/handwritten/06'
    },
    {
      text: '33 实现工具函数',
      link: '/docs/interview/handwritten/07'
    },
    {
      text: '34 手写常见排序',
      link: '/docs/interview/handwritten/08'
    },
    {
      text: '35 算法与数据结构',
      link: '/docs/interview/handwritten/09'
    },
    {
      text: '36 综合',
      link: '/docs/interview/handwritten/10'
    },
  ],
  '/docs/interview/principle/vue': [
    {
      text: '从源码解读Vue生命周期',
      link: '/docs/interview/principle/vue/01'
    },
    {
      text: '组件的本质',
      link: '/docs/interview/principle/vue/02'
    },
    {
      text: '有状态组件的设计',
      link: '/docs/interview/principle/vue/03'
    },
    {
      text: '设计 VNode',
      link: '/docs/interview/principle/vue/04'
    },
    {
      text: '辅助创建 VNode 的 h 函数',
      link: '/docs/interview/principle/vue/05'
    },
    {
      text: '自定义渲染器和异步渲染',
      link: '/docs/interview/principle/vue/06'
    },
    {
      text: '渲染器之挂载',
      link: '/docs/interview/principle/vue/07'
    },
    {
      text: '渲染器的核心 Diff 算法',
      link: '/docs/interview/principle/vue/08'
    },
    {
      text: '渲染器之patch',
      link: '/docs/interview/principle/vue/09'
    },
    {
      text: '图解 Vue 响应式原理',
      link: '/docs/interview/principle/vue/10'
    },
    {
      text: '图解 Vue 异步更新',
      link: '/docs/interview/principle/vue/11'
    },
    {
      text: '剖析 Vue 内部运行机制',
      link: '/docs/interview/principle/vue/12'
    },
    {
      text: 'vue响应式原理模拟',
      link: '/docs/interview/principle/vue/13'
    },
    {
      text: 'vue状态管理之vuex',
      link: '/docs/interview/principle/vue/14'
    },
    {
      text: '理解Vue的设计思想及实现Vue',
      link: '/docs/interview/principle/vue/15'
    },
    {
      text: 'diff算法深入',
      link: '/docs/interview/principle/vue/16'
    },
    {
      text: 'vue router vuex原理分析',
      link: '/docs/interview/principle/vue/17'
    },
    {
      text: 'Vue3初探响应式原理.',
      link: '/docs/interview/principle/vue/18'
    },
    {
      text: 'vue2源码分析',
      link: '/docs/interview/principle/vue/19'
    },
    {
      text: 'vue组件化实践',
      link: '/docs/interview/principle/vue/20'
    }

  ],
  '/docs/interview/principle/react': [
    {
      "text": "React router原理",
      "link": "/docs/interview/principle/react/01"
    },
    {
      "text": "Dva总结",
      "link": "/docs/interview/principle/react/02"
    },
    {
      "text": "MobX总结",
      "link": "/docs/interview/principle/react/03"
    },
    {
      "text": "浅析redux saga中间件及用法",
      "link": "/docs/interview/principle/react/04"
    },
    {
      "text": "Redux之浅析中间件",
      "link": "/docs/interview/principle/react/05"
    },
    {
      "text": "Redux之源码分析",
      "link": "/docs/interview/principle/react/06"
    },
    {
      "text": "Redux之异步Action及操作",
      "link": "/docs/interview/principle/react/07"
    },
    {
      "text": "浅析中间件",
      "link": "/docs/interview/principle/react/08"
    },
    {
      "text": "react结合redux实战",
      "link": "/docs/interview/principle/react/09"
    },
    {
      "text": "Immutable总结",
      "link": "/docs/interview/principle/react/10"
    },
    {
      "text": "React16为什么要更改生命周期上",
      "link": "/docs/interview/principle/react/11"
    },
    {
      "text": "React16为什么要更改生命周期下",
      "link": "/docs/interview/principle/react/12"
    },
    {
      "text": "React Hooks 设计动机与工作模式",
      "link": "/docs/interview/principle/react/13"
    },
    {
      "text": "深入 React Hooks 工作机制",
      "link": "/docs/interview/principle/react/14"
    },
    {
      "text": "真正理解虚拟DOM",
      "link": "/docs/interview/principle/react/15"
    },
    {
      "text": "React 中的“栈调和” Stack Reconciler 过程是怎样的",
      "link": "/docs/interview/principle/react/16"
    },
    {
      "text": "setState 到底是同步的，还是异步的",
      "link": "/docs/interview/principle/react/17"
    },
    {
      "text": "如何理解 Fiber 架构的迭代动机与设计思想",
      "link": "/docs/interview/principle/react/18"
    },
    {
      "text": "ReactDOM.render 是如何串联渲染链路的",
      "link": "/docs/interview/principle/react/19"
    },
    {
      "text": "剖析 Fiber 架构下 Concurrent 模式的实现原理",
      "link": "/docs/interview/principle/react/20"
    },
    {
      "text": "React 事件与 DOM 事件有何不同",
      "link": "/docs/interview/principle/react/21"
    },
    {
      "text": "揭秘 Redux 设计思想与工作原理",
      "link": "/docs/interview/principle/react/22"
    },
    {
      "text": "从 Redux 中间件实现原理切入，理解“面向切面编程”",
      "link": "/docs/interview/principle/react/23"
    },
    {
      "text": "如何打造高性能的 React 应用",
      "link": "/docs/interview/principle/react/24"
    },
    {
      "text": "跟 React 学设计模式",
      "link": "/docs/interview/principle/react/25"
    },
    {
      "text": "React全部api解读",
      "link": "/docs/interview/principle/react/26"
    },
    {
      "text": "Taro原理",
      "link": "/docs/interview/principle/react/27"
    }
  ],
  '/docs/interview/principle/webpack': [
    {
      "text": "Webpack4打包机制原理解析",
      "link": "/docs/interview/principle/webpack/01"
    },
    {
      "text": "webpack中的HMR热更新原理剖析",
      "link": "/docs/interview/principle/webpack/02"
    },
    {
      "text": "从源码窥探Webpack4.x原理",
      "link": "/docs/interview/principle/webpack/03"
    },
    {
      "text": "实现webpack小型打包工具",
      "link": "/docs/interview/principle/webpack/04"
    },
    {
      "text": "Babel原理及其使用",
      "link": "/docs/interview/principle/webpack/05"
    },
    {
      "text": "Webpack 与 Rollup 二者之间该如何选择",
      "link": "/docs/interview/principle/webpack/06"
    },
    {
      "text": "前端构建新玩法 Vite 上手与思考",
      "link": "/docs/interview/principle/webpack/07"
    },
    {
      "text": "利用 Webpack CodeSplitting 完成复杂应用拆包",
      "link": "/docs/interview/principle/webpack/08"
    },
    {
      "text": "玩转 Webpack 的 TreeShaking 与 sideEffects 特性",
      "link": "/docs/interview/principle/webpack/09"
    },
    {
      "text": "如何配置 Webpack SourceMap 的最佳实践",
      "link": "/docs/interview/principle/webpack/10"
    },
    {
      "text": "Webpack 运行机制与核心工作原理",
      "link": "/docs/interview/principle/webpack/11"
    }
  ],
  '/docs/interview/principle/node/': [
    {
      text: 'Node事件循环机制原理',
      link: '/docs/interview/principle/node/01'
    },
    {
      text: 'express详细使用',
      link: '/docs/interview/principle/node/02'
    },
    {
      text: 'koa基本用法',
      link: '/docs/interview/principle/node/03'
    }
  ],
  '/docs/interview/principle/javascript/': [
    {
      text: 'JavaScript 引擎如何执行 JavaScript 代码',
      link: '/docs/interview/principle/javascript/01'
    },
    {
      text: '单线程的 JavaScript 如何管理任务',
      link: '/docs/interview/principle/javascript/02'
    },
  ],
  '/docs/interview/principle/comprehensive': [
    {
      "text": "一、创建对象和原型链",
      "link": "/docs/interview/principle/comprehensive/01"
    },
    {
      "text": "二、面向对象",
      "link": "/docs/interview/principle/comprehensive/02"
    },
    {
      "text": "三、DOM事件总结",
      "link": "/docs/interview/principle/comprehensive/03"
    },
    {
      "text": "四、Event Loop详细版",
      "link": "/docs/interview/principle/comprehensive/04"
    },
    {
      "text": "五、CSS盒模型及BFC",
      "link": "/docs/interview/principle/comprehensive/05"
    },
    {
      "text": "六、页面布局",
      "link": "/docs/interview/principle/comprehensive/06"
    },
    {
      "text": "七、安全问题：CSRF和XSS",
      "link": "/docs/interview/principle/comprehensive/07"
    },
    {
      "text": "八、跨域通信类",
      "link": "/docs/interview/principle/comprehensive/08"
    },
    {
      "text": "九、前端错误监控",
      "link": "/docs/interview/principle/comprehensive/09"
    },
    {
      "text": "十、HTTP协议",
      "link": "/docs/interview/principle/comprehensive/10"
    }
  ],
  "/docs/interview/self-checking": [
    {
      text: '1-前端100题自检',
      link: "/docs/interview/self-checking/01"
    },
    {
      text: '2-面试综合汇总',
      link: "/docs/interview/self-checking/02"
    },
    {
      text: '3-Promise面试题',
      link: "/docs/interview/self-checking/03"
    }
  ],

  '/docs/interview/excellent-docs': [
    {
      "text": "1 HTML模块",
      "link": "/docs/interview/excellent-docs/01"
    },
    {
      "text": "2 CSS模块",
      "link": "/docs/interview/excellent-docs/02"
    },
    {
      "text": "3 JS模块",
      "link": "/docs/interview/excellent-docs/03"
    },
    {
      "text": "4 ES6模块",
      "link": "/docs/interview/excellent-docs/04"
    },
    {
      "text": "5 浏览器模块",
      "link": "/docs/interview/excellent-docs/05"
    },
    {
      "text": "6 React模块",
      "link": "/docs/interview/excellent-docs/06"
    },
    {
      "text": "7 Vue模块",
      "link": "/docs/interview/excellent-docs/07"
    },
    {
      "text": "8 Node模块",
      "link": "/docs/interview/excellent-docs/08"
    },
    {
      "text": "9 前端工程相关",
      "link": "/docs/interview/excellent-docs/09"
    },
    {
      "text": "10 移动多端开发",
      "link": "/docs/interview/excellent-docs/10"
    },
    {
      "text": "11 小程序模块",
      "link": "/docs/interview/excellent-docs/11"
    },
    {
      "text": "12 Uniapp模块",
      "link": "/docs/interview/excellent-docs/12"
    },
    {
      "text": "13 前端安全模块",
      "link": "/docs/interview/excellent-docs/13"
    },
    {
      "text": "14 性能优化相关",
      "link": "/docs/interview/excellent-docs/14"
    },
    {
      "text": "15 HTTP模块",
      "link": "/docs/interview/excellent-docs/15"
    },
    {
      "text": "16 常用设计模式",
      "link": "/docs/interview/excellent-docs/16"
    },
    {
      "text": "17 框架通识",
      "link": "/docs/interview/excellent-docs/17"
    },
    {
      "text": "18 排序算法",
      "link": "/docs/interview/excellent-docs/18"
    },
    {
      "text": "19 计算机通识",
      "link": "/docs/interview/excellent-docs/19"
    }
  ]
}